...
#+BEGIN_SRC emacs-lisp
  (use-package ef-themes :init (ef-themes-select 'ef-cyprus))
#+END_SRC
...
#+begin_src emacs-lisp
  (use-package olivetti :hook (org-mode . olivetti-mode) (markdown-mode . olivetti-mode))
#+end_src
...
#+begin_src emacs-lisp
  (use-package
   ...
   (org-todo-keywords
    '((sequence
       "TODO(t)" "HOLD(h!)" "WIP(i!)" "WAIT(w!)" "|" "DONE(d!)" "CANCELLED(c@/!)")
   ...
   (org-tag-persistent-alist
    '(("read" . ?r) ("mail" . ?m) ("emacs" . ?e) ("study" . ?s) ("work" . ?w)))
   ;; 预定义好的标签
   (org-tag-alist
    '((:startgroup)
      ("crypt" . ?c)
...
#+begin_src emacs-lisp
  (use-package
   org-modern
   :custom (org-modern-hide-stars 'leading)
   ;; 标题行型号字符
   (org-modern-star ["◉" "○" "✸" "✳" "◈" "◇" "✿" "❀" "✜"])
...
#+begin_src emacs-lisp
  (use-package
   go-translate
   :defer t
   :custom (gt-langs '(en zh) "set the lang translat")
   (gt-default-translator
    (gt-translator
     :engines
     (list
      (gt-youdao-dict-engine) (gt-google-rpc-engine :if 'not-word :cache nil))
...
#+begin_src emacs-lisp
  ...
  (use-package emacs :ensure nil :hook (org-mode . auto-space-mode) (markdown-mode . auto-space-mode))
#+end_src
